...
function validarConteudo(conteudo) {
  const estruturaValida = /^[A-Za-zÀ-ÿ\s]+$/.test(conteudo);
  const letrasReais = conteudo.replace(/[^A-Za-zÀ-ÿ]/g, "").length >= 10;
  return estruturaValida && letrasReais;
}
...
async function manipularFormulario(event) {
